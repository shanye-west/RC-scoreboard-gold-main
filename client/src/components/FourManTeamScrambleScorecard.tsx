...
// Team score interface for 4-Man Scramble
...
// Interface for the component props - matches 2-man Scramble for consistency
interface FourManScrambleScorecardProps {
  matchId: number;
  holes: any[];
...
/**
 * FourManTeamScrambleScorecard Component
 * 
 * Specialized scorecard for 4-man Team Scramble format
 * - Shows 4 players per team (display only)
 * - Only team scores are editable (not individual players)
 * - Includes match play status calculation
 * - Features enhanced grid-based UI matching Best Ball scorecard
 */
const FourManTeamScrambleScorecard: React.FC<FourManScrambleScorecardProps> = ({
  matchId,
  holes = [],
  aviatorPlayersList = [],
  producerPlayersList = [],
    ...
    <Card className="best-ball-scorecard-container">
      ...
      <CardContent>
        <div className="scorecard-grid">
          ...
          {/* Aviator Team Players (Display Only) */}
          {aviatorPlayersList.slice(0, 4).map((player, index) => (
            <React.Fragment key={`aviator-player-${player.id || index}`}>
              <div className="player-name aviator">{player.name || `Player ${index + 1}`}</div>
              <div className="empty"></div>
              {frontNine.map((hole) => (
                <div key={`aviator-${index}-${hole.number}`} className="empty"></div>
              ))}
              <div className="empty"></div>
              {backNine.map((hole) => (
                <div key={`aviator-${index}-${hole.number}`} className="empty"></div>
          ...
          {/* Producer Team Players (Display Only) */}
          {producerPlayersList.slice(0, 4).map((player, index) => (
            <React.Fragment key={`producer-player-${player.id || index}`}>
              <div className="player-name producers">{player.name || `Player ${index + 1}`}</div>
              <div className="empty"></div>
              {frontNine.map((hole) => (
                <div key={`producer-${index}-${hole.number}`} className="empty"></div>
              ))}
              <div className="empty"></div>
              {backNine.map((hole) => (
                <div key={`producer-${index}-${hole.number}`} className="empty"></div>
